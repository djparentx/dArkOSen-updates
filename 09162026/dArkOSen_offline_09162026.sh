...
if [ ! -f "/home/ark/.config/.dArkOSen-update09162026" ]; then
	...
	sleep 2
	if [ -f "/opt/system/Tools/dArkOSen-update09162026.zip" ]; then	
		sudo systemctl stop ogage.service
		# unzip
		unzip -X -o /opt/system/Tools/dArkOSen-update09162026.zip -d / | tee -a "$LOG_FILE"
		sleep 1
		# run update script
		bash /tmp/09162026.sh
	else
		printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." >> "$LOG_FILE" 2>&1
		rm -fv /opt/system/Tools/dArkOSen-update09162026.zip | tee -a "$LOG_FILE"
		sleep 3
		echo $c_brightness > /sys/class/backlight/backlight/brightness
		exit 1
	fi	
	
	rm -f /opt/system/Tools/dArkOSen-update09162026.zip
